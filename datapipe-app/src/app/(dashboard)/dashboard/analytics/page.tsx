            ...
            {isLoading ? <Skeleton className="h-16" /> : (
              <>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">{usage?.storage_used_mb} MB utilisés</span>
                  <span className="text-slate-500">{usage?.storage_limit_mb} MB</span>
                </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 pt-0">
            {isLoading || !aiUsage ? <Skeleton className="h-16" /> : (
              <>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">{formatNumber(aiUsage.tokens_used)} tokens</span>
                  <span className="text-slate-500">{formatNumber(aiUsage.tokens_limit)}</span>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 pt-1">
                  {Object.entries(aiUsage.breakdown).map(([k, v]) => (
                    <div key={k} className="rounded-md bg-[#ffffff] px-2 py-1.5 text-center">
                      <p className="text-[10px] text-slate-500 capitalize">{k.replace('_', ' ')}</p>
                      <p className="text-xs font-semibold text-slate-700">{formatNumber(v)}</p>
                    </div>
                  ))}
                </div>
              </>
